# Solid-OIDC client registration for MyPodApp
...
```
clientId: https://solidcommunity.au/apps/my_pod_app/client-profile.jsonld
```
...
- **If you maintain solidcommunity.au** (the ANU Solid Community), deploy these
  to `apps/my_pod_app/` alongside the other apps (this is how `filepod` is set
  up).
...
```bash
curl -I https://solidcommunity.au/apps/my_pod_app/client-profile.jsonld   # expect 200
```
...
`com.example.mypodapp://redirect` custom scheme.
